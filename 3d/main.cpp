...
class WaterGui : public Gui {
	...
	public:
		 ...
		 */
		void readConfig() {
			m_export_meshes = cfg.getExportMeshes();
			cfg.getSystemSize(m_system_size_x, m_system_size_y, m_system_size_z);
			cfg.getGridResolution(m_grid_res_x, m_grid_res_y, m_grid_res_z);
			m_jitter_particles = cfg.getJitterParticles();
			m_dt = cfg.getTimeStep();
			m_alpha = cfg.getAlpha();
			m_density = cfg.getDensity();
			m_gravity = cfg.getGravity();
			m_display_grid = cfg.getDisplayGrid();
			m_max_p_disp = cfg.getMaxParticlesDisplay();
		}
		 ...
		 */
		void updateConfig() {
			cfg.setExportMeshes(m_export_meshes);
			cfg.setSystemSize(m_system_size_x, m_system_size_y, m_system_size_z);
			cfg.setGridResolution(m_grid_res_x, m_grid_res_y, m_grid_res_z);
			cfg.setJitterParticles(m_jitter_particles);
			cfg.setTimeStep(m_dt);
			cfg.setAlpha(m_alpha);
			cfg.setDensity(m_density);
			cfg.setGravity(m_gravity);
			cfg.setDisplayGrid(m_display_grid);
			cfg.setMaxParticlesDisplay(m_max_p_disp);
		}
